...
func (e *DuplicateFareTransferRuleError) Error() string {
	return fmt.Sprintf(
		"fare_transfer_rule with fare_product_id '%s' from_leg_group_id '%s' to_leg_group_id '%s' filter_fare_product_id '%s' transfer_count %d and duration_limit %d is duplicated",
		e.FareProductID,
		e.FromLegGroupID,
		e.ToLegGroupID,
		e.FilterFareProductID,
		e.TransferCount,
		e.DurationLimit,
	)
}
...
func (e *DuplicateFareTransferRuleCheck) Validate(ent tt.Entity) []error {
	v, ok := ent.(*gtfs.FareTransferRule)
	if !ok {
		return nil
	}
	if e.vals == nil {
