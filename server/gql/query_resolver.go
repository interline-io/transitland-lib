...
func (r *queryResolver) CensusValues(ctx context.Context, first *int, after *string, where *model.CensusValueFilter) (*model.CensusValueConnection, error) {
	cfg := model.ForContext(ctx)
	...
	values, err := cfg.Finder.FindCensusValues(ctx, &fetchLimit, cursor, where)
	if err != nil {
		return nil, err
	}
