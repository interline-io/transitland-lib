...
func (r *validationReportResolver) Errors(ctx context.Context, obj *model.ValidationReport, limit *int) ([]*model.ValidationReportErrorGroup, error) {
	...
	return LoaderFor(ctx).ValidationReportErrorGroupsByValidationReportIDs.Load(ctx, validationReportErrorGroupLoaderParam{ValidationReportID: obj.ID, Limit: resolverCheckLimit(limit)})()
}
...
func (r *validationReportErrorGroupResolver) Errors(ctx context.Context, obj *model.ValidationReportErrorGroup, limit *int) ([]*model.ValidationReportError, error) {
	...
	ret, err := LoaderFor(ctx).ValidationReportErrorExemplarsByValidationReportErrorGroupIDs.Load(ctx, validationReportErrorExemplarLoaderParam{ValidationReportGroupID: obj.ID, Limit: resolverCheckLimit(limit)})()
	if err != nil {
		return nil, err
	}
	for _, r := range ret {
