...
func TestSetupTestDB_WithoutURL(t *testing.T) {
	// Test setupTestDB without URL - should only create feed, no feed_version
	adapter, feedID, feedVersionID := setupTestDB(t, "f-test~no-version", "")
	defer adapter.Close()

	assert.Greater(t, feedID, 0, "should have created a feed")
	assert.Equal(t, 0, feedVersionID, "should not have created a feed version")

	ctx := context.Background()

	// Verify feed was created with correct onestop_id
	var onestopID string
	err := dbutil.Get(ctx, adapter.DBX(), adapter.Sqrl().
		Select("onestop_id").
		From("current_feeds").
		Where("id = ?", feedID), &onestopID)
	require.NoError(t, err, "failed to query feed")
	assert.Equal(t, "f-test~no-version", onestopID, "wrong onestop_id")

	// Verify feed_state was created
	var feedStateCount int
	err = dbutil.Get(ctx, adapter.DBX(), adapter.Sqrl().
		Select("COUNT(*)").
		From("feed_states").
		Where("feed_id = ?", feedID), &feedStateCount)
	require.NoError(t, err, "failed to count feed states")
	assert.Equal(t, 1, feedStateCount, "should have created feed state")

	// Verify no feed version was created
	var versionCount int
	err = dbutil.Get(ctx, adapter.DBX(), adapter.Sqrl().
		Select("COUNT(*)").
		From("feed_versions").
		Where("feed_id = ?", feedID), &versionCount)
	require.NoError(t, err, "failed to count feed versions")
	assert.Equal(t, 0, versionCount, "should have no feed versions")
}
