...
// CreateTestFeedVersionImport returns an import record for a feed version, inserted into a
// database.
func CreateTestFeedVersionImport(atx tldb.Adapter, fvid int, success bool, inProgress bool) dmfr.FeedVersionImport {
	fvi := dmfr.FeedVersionImport{Success: success, InProgress: inProgress}
	fvi.FeedVersionID = fvid
	fvi.ID = MustInsert(atx, &fvi)
	return fvi
}
