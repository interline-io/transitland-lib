...
func (ent *Translation) Errors() (errs []error) {
	...
	errs = append(errs, enum.CheckPresent("table_name", ent.TableNameValue.Val)...)
	errs = append(errs, enum.CheckPresent("field_name", ent.FieldName.Val)...)
	errs = enum.CheckError(errs, enum.CheckFieldPresentError("language", &ent.Language))
	errs = append(errs, enum.CheckPresent("translation", ent.Translation.Val)...)
	errs = append(errs, enum.CheckInArray("table_name", ent.TableNameValue.Val, "agency", "routes", "stops", "trips", "stop_times", "pathways", "levels", "feed_info", "attributions")...)
	// RecordID
	if ent.RecordID.Val == "" {
