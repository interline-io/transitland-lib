...
func (pp *SegmentBuilder) SetGeomCache(g *xy.GeomCache) {
	pp.geomCache = g
}
	...
	for _, pat := range pp.stopPatterns {
		...
		for i := 0; i < len(pat.Pattern)-1; i++ {
			...
			if len(shapeInfo.Line) > 0 {
				...
				// )
				sliceLine = xy.LineSlicePoints(shapeInfo.Line, spt, ept)
			}
			// fmt.Println(debugLine(sliceLine, spt, ept))
			if sliceLineDist, ptDist := xy.LengthHaversine(sliceLine), xy.DistanceHaversinePoint(spt, ept); sliceLineDist < ptDist {
