...
func TestLoadAndParseRegistry_from_file(t *testing.T) {
	parsedContents, err := LoadAndParseRegistry(testutil.RelPath("testdata/dmfr/example.json"))
	if err != nil {
		t.Error(err)
	}
...
func TestLoadAndParseRegistry_from_URL(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		buf, err := ioutil.ReadFile(testutil.RelPath("testdata/dmfr/example.json"))
		if err != nil {
			t.Error(err)
		}
		w.Write(buf)
...
func TestLoadAndParseRegistry_Secrets(t *testing.T) {
	parsedContents, err := LoadAndParseRegistry(testutil.RelPath("testdata/dmfr/secrets.json"))
	if err != nil {
		t.Error(err)
	}
...
func TestImplicitOperatorInFeed(t *testing.T) {
	reg, err := LoadAndParseRegistry(testutil.RelPath("testdata/dmfr/embedded.json"))
	if err != nil {
		t.Fatal(err)
	}
	tcs := []struct {
