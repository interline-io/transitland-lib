...
func TestSecretsMatchFilename(t *testing.T) {
	reg, err := dmfr.LoadAndParseRegistry(testutil.RelPath("test/data/dmfr/secrets.json"))
	if err != nil {
		t.Fatal(err)
	}
	testcases := []struct {
	...
	for _, tc := range testcases {
		t.Run(tc.match, func(t *testing.T) {
			found, err := SecretsMatchFilename(reg.Secrets, tc.match)
...
func TestSecretsMatchFeed(t *testing.T) {
	reg, err := dmfr.LoadAndParseRegistry(testutil.RelPath("test/data/dmfr/secrets.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range testcases {
		t.Run(tc.match, func(t *testing.T) {
			found, err := SecretsMatchFeed(reg.Secrets, tc.match)
			if err != nil && tc.expecterr == false {
				t.Errorf("got unexpected error: %s", err)
			} else if err == nil && tc.expecterr == true {
				t.Errorf("expected error")
