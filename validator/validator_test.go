...
func TestSaveValidationReport(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		buf, err := os.ReadFile(testutil.RelPath("test/data/rt/ct-trip-updates.pb.json"))
		if err != nil {
			t.Error(err)
		}
		w.Write(buf)
	...
	opts := Options{
		...
		EvaluateAt:                   now,
		ValidateRealtimeMessages:     []string{ts.URL},
	}
	...
	testdb.TempSqlite(func(atx tldb.Adapter) error {
		if err := SaveValidationReport(atx, result, 1, true, true); err != nil {
			t.Fatal(err)
		}
		return nil
	})
