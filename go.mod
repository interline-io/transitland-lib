module github.com/interline-io/transitland-lib

go 1.16

require (
	github.com/99designs/gqlgen v0.13.0 // indirect
	github.com/Masterminds/squirrel v1.5.0
	github.com/aws/aws-sdk-go v1.38.54 // indirect
	github.com/dimchansky/utfbom v1.1.0
	github.com/flopp/go-staticmaps v0.0.0-20210425143944-2e6e19a99c28 // indirect
	github.com/form3tech-oss/jwt-go v3.2.3+incompatible // indirect
	github.com/golang/geo v0.0.0-20210108004804-a63082ebfb66 // indirect
	github.com/golang/protobuf v1.4.1
	github.com/gookit/color v1.2.0
	github.com/gorilla/handlers v1.5.1 // indirect
	github.com/gorilla/mux v1.8.0
	github.com/hypirion/go-filecache v0.0.0-20160810125507-e3e6ef6981f0 // indirect
	github.com/jlaffaye/ftp v0.0.0-20191218041957-e1b8fdd0dcc3
	github.com/jmoiron/sqlx v1.3.1
	github.com/lib/pq v1.8.0
	github.com/machinebox/graphql v0.2.2 // indirect
	github.com/mattn/go-sqlite3 v1.14.6
	github.com/mmcloughlin/geohash v0.10.0
	github.com/snabb/isoweek v1.0.0
	github.com/tidwall/gjson v1.8.0 // indirect
	github.com/twpayne/go-geom v1.3.6
	github.com/vektah/gqlparser/v2 v2.1.0 // indirect
	google.golang.org/protobuf v1.25.0
)
